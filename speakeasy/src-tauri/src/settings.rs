...
pub enum ClipboardHandling {
    // DontModify swaps the transcript onto the clipboard, sends Cmd/Ctrl+V, then
    // restores the prior clipboard after a delay. That restore RACES slow-paste
    // apps (Electron: cmux, VS Code) which process the paste AFTER the restore, so
    // they receive the OLD clipboard. CopyToClipboard leaves the transcript on the
    // clipboard (no restore) — proven reliable across Notes/Chrome/cmux live
    // 2026-07-17. SpeakEasy defaults to it; DontModify is opt-in for users who must
    // preserve their clipboard and accept the Electron paste race.
    DontModify,
    #[default]
    CopyToClipboard,
}
...
/// How aggressively to clean up a transcript before it is pasted (LocalFlow's
/// "polish" pipeline — see `crate::polish`). Levels stack:
/// - `None`: paste the raw transcript untouched.
/// - `Light`: deterministic pass only (filler-word strip, repeated-word
///   collapse, whitespace/caps cleanup). No network, no LLM.
/// - `Medium`: light + a local-ollama LLM pass (grammar, punctuation,
///   false-start resolution).
/// - `High`: medium with a richer prompt (adds light tone smoothing).
///
/// The LLM pass is best-effort: if ollama is unreachable, medium/high silently
/// degrade to the deterministic (light) result — a transcript is never blocked
/// or queued for later sending (egress policy).
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Type, Default)]
#[serde(rename_all = "lowercase")]
...
impl PolishLevel {
    /// Whether this level runs the local-ollama LLM pass on top of the
    /// deterministic cleanup.
    pub fn uses_llm(self) -> bool {
        matches!(self, PolishLevel::Medium | PolishLevel::High)
    }
}

/// LocalFlow egress guard: a post-process / LLM base URL may only target the
/// local machine. Accepts `http(s)://127.0.0.1[:port]`, `[::1]`, or `localhost`
/// (with optional userinfo/path). Anything else — a cloud host, a LAN IP — is
/// rejected at settings-write time so no transcript can leave the device through
/// the LLM path.
...
impl Default for KeyboardImplementation {
    fn default() -> Self {
        // macOS: the handy-keys IOKit HID listener did NOT deliver key events on
        // macOS 26 even with Input Monitoring granted (verified live 2026-07-17);
        // the Tauri global-shortcut engine captured the hotkey immediately. Linux
        // has always used Tauri. Windows keeps handy-keys (unchanged, untested here).
...
pub struct AppSettings {
    ...
    /// Transcript cleanup level applied before paste (deterministic + optional
    /// local-ollama LLM). See [`PolishLevel`].
    #[serde(default)]
    pub polish_level: PolishLevel,
    /// The ollama model used for the medium/high polish LLM pass. Shown from
    /// ollama's model list in the UI; only consulted when `polish_level` is
    /// medium/high.
    #[serde(default = "default_polish_ollama_model")]
    pub polish_ollama_model: String,
    /// Hard wall-clock budget (ms) for the medium/high polish LLM pass. If ollama
    /// doesn't answer within it, polish silently falls back to the deterministic
    /// result so a paste is never blocked. On low-RAM/CPU-only boxes local models
    /// can take minutes; 5s keeps the paste snappy.
    #[serde(default = "default_polish_llm_timeout_ms")]
    pub polish_llm_timeout_ms: u64,
    /// Opt-in advanced mode: hold the spacebar to dictate. Off by default — it
    /// suppresses the space key, which is risky in secure-input fields, so it
    /// ships behind a warning. See [`crate::shortcut::spacebar`].
    #[serde(default)]
    pub spacebar_ptt: bool,
    /// How long (ms) the spacebar must be held before it starts recording. A
    /// quick tap under this threshold types a normal space instead.
    #[serde(default = "default_spacebar_hold_threshold_ms")]
    pub spacebar_hold_threshold_ms: u64,
    /// Keep the overlay visible at idle as a persistent Wispr-style pill/dot
    /// (click-through). When false, the overlay only appears during an active
    /// recording/transcription. Ignored when `overlay_style` is None.
...
fn default_push_to_talk() -> bool {
    // SpeakEasy default: TOGGLE mode (tap the hotkey to start, tap again to stop)
    // with the live pill indicator — no holding a key for the whole dictation.
    false
}
...
fn default_paste_delay_ms() -> u64 {
    // Give the target app a beat to receive focus/paste before Cmd+V. 60ms was
    // too tight for slower apps; 100ms proved reliable live 2026-07-17.
    100
}

fn default_paste_delay_after_ms() -> u64 {
    // Only matters on the DontModify path (delay before clipboard restore).
    120
}
...
/// LocalFlow egress policy: exactly one post-processing/LLM provider ships — a
/// **local ollama** server on `127.0.0.1:11434` (OpenAI-compatible `/v1`
/// surface). All cloud presets (OpenAI, Z.AI, OpenRouter, Anthropic, Groq,
/// Cerebras, Bedrock) and the Apple Intelligence adapter were removed; no
/// transcript can be sent off-device. The base URL stays editable but is
/// validated to localhost on write (see [`is_localhost_url`]).
...
fn default_polish_ollama_model() -> String {
    "qwen3:4b".to_string()
}

fn default_polish_llm_timeout_ms() -> u64 {
    5000
}

fn default_spacebar_hold_threshold_ms() -> u64 {
    350
}
...
fn ensure_post_process_defaults(settings: &mut AppSettings) -> bool {
    ...
    // LocalFlow egress enforcement: drop any provider whose base URL is not
    // localhost — e.g. a cloud preset persisted by a pre-fork store. This runs on
    // every settings read, so the policy can't be re-introduced by an old file.
    let before = settings.post_process_providers.len();
    settings
        .post_process_providers
        .retain(|p| is_localhost_url(&p.base_url));
    ...
    // Repair the active selection if it pointed at a now-removed provider.
...
pub fn get_default_settings() -> AppSettings {
    ...
    AppSettings {
        ...
        polish_level: PolishLevel::default(),
        polish_ollama_model: default_polish_ollama_model(),
        polish_llm_timeout_ms: default_polish_llm_timeout_ms(),
        spacebar_ptt: false,
        spacebar_hold_threshold_ms: default_spacebar_hold_threshold_ms(),
        show_idle_pill: default_show_idle_pill(),
    }
}
...
mod tests {
    ...
    #[test]
    fn localhost_urls_are_accepted() {
        for url in [
    ...
    #[test]
    fn default_provider_is_local_ollama_only() {
        let settings = get_default_settings();
        assert_eq!(settings.post_process_providers.len(), 1);
        let provider = &settings.post_process_providers[0];
        assert_eq!(provider.id, "ollama");
        assert!(is_localhost_url(&provider.base_url));
        assert_eq!(settings.post_process_provider_id, "ollama");
    }

    #[test]
    fn ensure_defaults_strips_persisted_cloud_provider() {
        let mut settings = get_default_settings();
        // Simulate a pre-fork store that still carries a cloud preset + selection.
        ...
        assert!(settings
            .post_process_providers
            .iter()
            .all(|p| is_localhost_url(&p.base_url)));
        assert!(!settings
            .post_process_providers
            .iter()
            .any(|p| p.id == "openai"));
    ...
    #[test]
    fn polish_level_defaults_to_light() {
        assert_eq!(PolishLevel::default(), PolishLevel::Light);
        assert!(!PolishLevel::Light.uses_llm());
        assert!(!PolishLevel::None.uses_llm());
        assert!(PolishLevel::Medium.uses_llm());
        assert!(PolishLevel::High.uses_llm());

        let settings = get_default_settings();
        assert_eq!(settings.polish_level, PolishLevel::Light);
        assert_eq!(settings.polish_ollama_model, "qwen3:4b");
        assert_eq!(settings.polish_llm_timeout_ms, 5000);
        assert!(!settings.spacebar_ptt);
        assert_eq!(settings.spacebar_hold_threshold_ms, 350);
