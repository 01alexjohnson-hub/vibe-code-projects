...
pub(crate) async fn process_transcription_output(
    ...
    if post_process {
        ...
        // LocalFlow polish pass — the default cleanup path. Governed by
        // `polish_level`: deterministic for light, plus a local-ollama LLM pass
        // for medium/high (which degrades to deterministic if ollama is down).
        // The raw transcript (`transcription`) is preserved as-is in history; the
        // polished result becomes the pasted text and the stored "polished"
        // variant, so the UI can offer "Undo AI edit".
        let polished = crate::polish::polish(
            &final_text,
            settings.polish_level,
            &settings.polish_ollama_model,
            settings.polish_llm_timeout_ms,
        )
        .await;
        if polished != final_text {
            final_text = polished;
        }
