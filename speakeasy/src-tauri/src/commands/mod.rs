...
pub fn initialize_shortcuts(app: AppHandle) -> Result<(), String> {
    ...
    // Start the opt-in spacebar push-to-talk grab tap if enabled. Done here (not
    // at startup) so it only requests Input-Monitoring after permissions/onboarding,
    // and only when the user has actually turned it on.
    let settings = get_settings(&app);
    crate::shortcut::spacebar::apply(
        &app,
        settings.spacebar_ptt,
        settings.spacebar_hold_threshold_ms,
    );
