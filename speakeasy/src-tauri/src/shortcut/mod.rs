...
use crate::settings::{
    ...
    OverlayPosition, OverlayStyle, PasteMethod, PolishLevel, ShortcutBinding, SoundTheme, Theme,
...
pub fn change_post_process_base_url_setting(
    ...
    // LocalFlow egress policy: a post-process base URL may only point at the
    // local machine. Reject anything else here, at write time, so a remote host
    // can never be persisted (defense in depth alongside the load-time prune).
    if !settings::is_localhost_url(&base_url) {
        return Err(
            "Base URL must point at localhost (127.0.0.1 or localhost) — LocalFlow never sends transcripts off-device."
                .to_string(),
        );
    }
...
#[tauri::command]
#[specta::specta]
pub fn change_polish_level_setting(app: AppHandle, level: String) -> Result<(), String> {
    let parsed = match level.as_str() {
        "none" => PolishLevel::None,
        "light" => PolishLevel::Light,
        "medium" => PolishLevel::Medium,
        "high" => PolishLevel::High,
        other => {
            warn!("Invalid polish level '{}', defaulting to light", other);
            PolishLevel::Light
        }
    };
    let mut settings = settings::get_settings(&app);
    settings.polish_level = parsed;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_polish_ollama_model_setting(app: AppHandle, model: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.polish_ollama_model = model;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_polish_llm_timeout_setting(app: AppHandle, ms: u64) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.polish_llm_timeout_ms = ms;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_spacebar_ptt_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.spacebar_ptt = enabled;
    let threshold = settings.spacebar_hold_threshold_ms;
    settings::write_settings(&app, settings);

    // Start/stop the spacebar grab tap to match the new setting immediately.
    spacebar::apply(&app, enabled, threshold);
    Ok(())
}

#[tauri::command]
#[specta::specta]
pub fn change_spacebar_hold_threshold_setting(app: AppHandle, ms: u64) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.spacebar_hold_threshold_ms = ms;
    let enabled = settings.spacebar_ptt;
    settings::write_settings(&app, settings);

    // Push the new threshold into the running tap (no-op if disabled).
    spacebar::apply(&app, enabled, ms);
    Ok(())
}

#[tauri::command]
#[specta::specta]
pub fn change_show_idle_pill_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.show_idle_pill = enabled;
    settings::write_settings(&app, settings);

    // Reflect the change immediately: show the idle pill, or hide the overlay.
    if enabled {
        crate::overlay::show_idle_overlay(&app);
    } else {
        crate::overlay::hide_idle_overlay(&app);
    }
    Ok(())
}
