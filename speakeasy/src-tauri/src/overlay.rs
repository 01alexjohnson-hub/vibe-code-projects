...
fn show_overlay_state(app_handle: &AppHandle, state: &str) {
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        // The persistent idle pill must be click-through so it never steals a
        // click on whatever is behind it; active states stay interactive.
...
/// Show the persistent idle pill (Wispr-style dot) when `show_idle_pill` is on
/// and the overlay isn't disabled. Uses the same `show-overlay` channel as every
/// active state, with the `"idle"` state string, so the frontend's overlay state
/// machine renders the dot. The idle pill is click-through (see `show_overlay_state`).
pub fn show_idle_overlay(app_handle: &AppHandle) {
    let settings = settings::get_settings(app_handle);
    if settings.show_idle_pill && settings.overlay_style != OverlayStyle::None {
...
/// Emit the fade-out and hide the overlay window after the animation window.
...
/// Ends the active overlay state. With the idle pill enabled we return to the
/// persistent idle dot instead of hiding the window entirely; otherwise the
/// window fades out and hides (the pre-idle-pill behavior).
...
/// Hide the idle pill entirely — used when the user turns `show_idle_pill` off.
pub fn hide_idle_overlay(app_handle: &AppHandle) {
    hide_overlay_window(app_handle);
}
