...
// Guards refreshSettings against a hung IPC call (getAppSettings that never
// resolves or rejects) — without this, isLoading could stay true forever,
// leaving the Hub on an indefinite blank/loading state instead of the
// defaults-with-a-retry-banner fallback below.
const GET_SETTINGS_TIMEOUT_MS = 8000;

interface SettingsStore {
  settings: Settings | null;
  defaultSettings: Settings | null;
  isLoading: boolean;
  /** True once a getAppSettings call has failed or timed out; cleared on the
   * next successful refresh. Drives the "couldn't load settings" banner. */
...
const settingUpdaters: {
  ...
  show_idle_pill: (value) =>
    commands.changeShowIdlePillSetting(value as boolean),
  ...
  polish_level: (value) => commands.changePolishLevelSetting(value as string),
  polish_ollama_model: (value) =>
    commands.changePolishOllamaModelSetting(value as string),
  spacebar_ptt: (value) => commands.changeSpacebarPttSetting(value as boolean),
  spacebar_hold_threshold_ms: (value) =>
    commands.changeSpacebarHoldThresholdSetting(value as number),
  polish_llm_timeout_ms: (value) =>
    commands.changePolishLlmTimeoutSetting(value as number),
};
...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    // Load settings from store. Always resolves isLoading to false and never
    // throws — a failed or hung fetch degrades to settingsLoadFailed=true
    // (the Hub shows a retry banner and runs on defaults) rather than
    // leaving the app on an indefinite blank/loading screen.
    refreshSettings: async () => {
      try {
        const timeout = new Promise<never>((_, reject) => {
          setTimeout(
            () => reject(new Error("getAppSettings timed out")),
            GET_SETTINGS_TIMEOUT_MS,
          );
        });
        const result = await Promise.race([commands.getAppSettings(), timeout]);
        if (result.status === "ok") {
          ...
          set({ isLoading: false, settingsLoadFailed: true });
        }
      } catch (error) {
