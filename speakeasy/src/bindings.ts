...
async changePolishLevelSetting(level: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_polish_level_setting", { level }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changePolishOllamaModelSetting(model: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_polish_ollama_model_setting", { model }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changePolishLlmTimeoutSetting(ms: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_polish_llm_timeout_setting", { ms }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changeSpacebarPttSetting(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_spacebar_ptt_setting", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changeSpacebarHoldThresholdSetting(ms: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_spacebar_hold_threshold_setting", { ms }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changeShowIdlePillSetting(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_show_idle_pill_setting", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
/**
 * The ollama model used for the medium/high polish LLM pass.
 */
polish_ollama_model?: string;
/**
 * Hard wall-clock budget (ms) for the medium/high polish LLM pass before it
 * silently falls back to the deterministic result.
 */
polish_llm_timeout_ms?: number;
/**
 * Opt-in advanced mode: hold the spacebar to dictate.
 */
spacebar_ptt?: boolean;
/**
 * How long (ms) the spacebar must be held before it starts recording.
 */
spacebar_hold_threshold_ms?: number;
/**
...
export type PolishLevel = "none" | "light" | "medium" | "high"
